...
namespace ParkwayNorthEvents.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (role == "") { role = SD.ClientRole; } //make the first login a client)
            ...
            if (ModelState.IsValid)
            {
                ...
                IList<string> usersRole = new List<string>();
                if (user != null)
                {
                    usersRole = await _userManager.GetRolesAsync(user);
                }
                ...
                else if (usersRole.Any(x => x.Equals(SD.AdminRole) || x.Equals(SD.ClientRole) || x.Equals(SD.ManagerRole)))
                {
                    result = IdentityResult.Failed(); 
                }
                else // user exists as unregistered client 
                {
                    await _userManager.RemoveFromRoleAsync(user, SD.UnregisteredClient);
                    var hasher = new PasswordHasher<ApplicationUser>();
                    user.FirstName = Input.FirstName;
                    user.LastName = Input.LastName;
                    user.PhoneNumber = Input.PhoneNumber;
                    user.PasswordHash = hasher.HashPassword(null, Input.Password);
